...
if file_to_process is not None:
    try:
        ...
        count_columns = [
            #"main_character", "dominance_gender", "dominance_age", "dominance_expression",
            "Sukarno", "Suharto", "B.J. Habibie", "Abdurrahman Wahid", "Megawati Sukarnoputri",
            "Susilo Bambang Yudhoyono", "Joko Widodo", "Prabowo Subianto", "Anies Baswedan",
            "Ganjar Pranowo", "Gibran Rakabuming Raka", "Ma’ruf Amin", "Airlangga Hartarto",
            "Sri Mulyani Indrawati", "Erick Thohir", "Agus Harimurti Yudhoyono", "Unknown Person",
            "male", "female",
        ...
        st.subheader("Raw Data Preview (with Color Coding)")

        numeric_cols = df.select_dtypes(include=np.number).columns.tolist()[:-2]

        # --- Generate and display the colorbar ---
        # Adjusted figsize to make the colorbar smaller (height reduced from 0.5 to 0.2)
        # fig, ax = plt.subplots(figsize=(6, 0.2))
        # cmap = plt.cm.Blues
        # norm = mcolors.Normalize(vmin=df[numeric_cols].min().min(), vmax=df[numeric_cols].max().max())

        # cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap),
        #                     cax=ax, orientation='horizontal', shrink=1.5)
        ...
        # st.image(buf, caption="Color Scale for Raw Data", use_container_width=True)
        # --- End of colorbar generation ---

        st.dataframe(df.style.background_gradient(cmap='Blues', subset=numeric_cols))
